...
func create_curve_point_basis(point_a: Vector3, point_b: Vector3, tilt: float) -> Basis:
	...
	if !curve_point_basis:
		...
		curve_point_basis = Basis.looking_at(projection * -1, Vector3.DOWN, false)
...
func generate_ramp_surface():
	...
	for i in ramp_surface_points.size() - 2:
		...
		var debug_marker = Marker3D.new()
		debug_marker.set_transform(Transform3D(curve_point_basis_a, point_a))
		debug_marker.set_owner(self)
		self.add_child(debug_marker)
		debug_markers.append(debug_marker)
		...
		var bottom_right_point_1 = right_edge_point_1 + ramp_thickness_vector * curve_point_basis_a
		var bottom_left_point_1 = left_edge_point_1  + ramp_thickness_vector * curve_point_basis_a
		var bottom_right_point_2 = right_edge_point_2  + ramp_thickness_vector * curve_point_basis_b
		var bottom_left_point_2 = left_edge_point_2  + ramp_thickness_vector * curve_point_basis_b
		...
		# start endcap
		if i == 0:
			ramp_entrance.transform = Transform3D(curve_point_basis_a, point_a)
			st.add_vertex(right_edge_point_1)
			st.add_vertex(bottom_right_point_1)
			st.add_vertex(bottom_left_point_1)
		...
		if i == ramp_surface_points.size() - 3:
			ramp_exit.transform = Transform3D(curve_point_basis_b, point_b)
			st.add_vertex(right_edge_point_2)
			st.add_vertex(bottom_left_point_2)
			st.add_vertex(bottom_right_point_2)
