...
@onready var resting_transform = self.transform
@onready var resting_origin = self.global_transform.origin
@onready var resting_euler = self.transform.basis.get_euler()
...
func _integrate_forces(state):
	...
	var local_origin = state_origin * transform.basis.inverse()
	...
	state_origin = local_origin * transform.basis

	var state_basis = state_transform.basis
	var local_basis = state_basis * transform.basis.inverse()
	...
	state_basis = Basis.from_euler(local_euler * transform.basis)
